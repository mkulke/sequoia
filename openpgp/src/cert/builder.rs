...
impl CertBuilder {
    ...
    pub fn generate(self) -> Result<(Cert, Signature)> {
        // XXX: I think this function should be async to support
        // changing certificates with detached keys.
        futures::executor::block_on(self.generate_async())
    }

    async fn generate_async(mut self) -> Result<(Cert, Signature)> {
        use crate::Packet;
        ...
        let (primary, sig) = self.primary_key(creation_time).await?;
        ...
        for (i, uid) in self.userids.into_iter().enumerate() {
            ...
            let signature = uid.bind(&mut signer, &cert, builder).await?;
        ...
        for (i, ua) in self.user_attributes.into_iter().enumerate() {
            ...
            let signature = ua.bind(&mut signer, &cert, builder).await?;
            cert = cert.insert_packets(
        ...
        for blueprint in self.subkeys {
            ...
            let signature = subkey.bind(&mut signer, &cert, builder).await?;
        ...
        let revocation = CertRevocationBuilder::new()
            ...
            .build(&mut signer, &cert, None).await?;
    ...
    async fn primary_key(&self, creation_time: std::time::SystemTime)
        -> Result<(key::SecretKey, Signature)>
    {
        ...
        let sig = sig.sign_direct_key(&mut signer, &key).await?;
