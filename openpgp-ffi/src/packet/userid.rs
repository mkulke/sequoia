...
#[crate::ffi_wrapper_type(prefix = "pgp_", derive = "Clone, Debug, PartialEq")]
...
#[::sequoia_ffi_macros::extern_fn]
#[no_mangle]
pub extern "C" fn pgp_user_id_new(value: *const c_char) -> *mut UserID {
    let value: &[u8] = ffi_param_cstr!(value).to_bytes();
    openpgp::packet::UserID::from(value).move_into_raw()
}
...
#[::sequoia_ffi_macros::extern_fn]
#[no_mangle]
pub extern "C" fn pgp_user_id_from_address(
    ...
    ffi_try!(openpgp::packet::UserID::from_address(
...
#[::sequoia_ffi_macros::extern_fn]
#[no_mangle]
pub extern "C" fn pgp_user_id_from_unchecked_address(
    errp: Option<&mut *mut crate::error::Error>,
    name: Option<&c_char>,
    comment: Option<&c_char>,
    address: *const c_char,
...
#[::sequoia_ffi_macros::extern_fn]
#[no_mangle]
pub extern "C" fn pgp_user_id_from_raw(value: *const u8, len: size_t) -> *mut UserID {
...
#[::sequoia_ffi_macros::extern_fn]
#[no_mangle]
pub extern "C" fn pgp_user_id_into_packet(uid: *mut UserID) -> *mut Packet {
    let p: openpgp::Packet = uid.move_from_raw().into();
    p.move_into_raw()
}
...
pub extern "C" fn pgp_user_id_name(
    ...
    match uid.name() {
        Ok(Some(name)) => *namep = ffi_return_string!(name),
        Ok(None) => *namep = ::std::ptr::null_mut(),
        Err(err) => {
            use crate::MoveIntoRaw;
...
pub extern "C" fn pgp_user_id_comment(
    ...
    match uid.comment() {
        Ok(Some(comment)) => *commentp = ffi_return_string!(comment),
        Ok(None) => *commentp = ::std::ptr::null_mut(),
        Err(err) => {
            use crate::MoveIntoRaw;
            let status = crate::error::Status::from(&err);
            if let Some(errp) = errp {
...
#[::sequoia_ffi_macros::extern_fn]
#[no_mangle]
pub extern "C" fn pgp_user_id_email(
    errp: Option<&mut *mut crate::error::Error>,
    uid: *const UserID,
    ...
    match uid.email() {
        Ok(Some(address)) => *addressp = ffi_return_string!(address),
        Ok(None) => *addressp = ::std::ptr::null_mut(),
        Err(err) => {
            use crate::MoveIntoRaw;
            let status = crate::error::Status::from(&err);
            if let Some(errp) = errp {
...
#[::sequoia_ffi_macros::extern_fn]
#[no_mangle]
pub extern "C" fn pgp_user_id_uri(
    errp: Option<&mut *mut crate::error::Error>,
    uid: *const UserID,
    urip: &mut *mut c_char,
) -> Status {
    ...
    match uid.uri() {
        Ok(Some(uri)) => *urip = ffi_return_string!(uri),
        Ok(None) => *urip = ::std::ptr::null_mut(),
        Err(err) => {
            use crate::MoveIntoRaw;
            let status = crate::error::Status::from(&err);
            if let Some(errp) = errp {
...
pub extern "C" fn pgp_user_id_email_normalized(
    ...
    match uid.email_normalized() {
        Ok(Some(email)) => *emailp = ffi_return_string!(email),
        Ok(None) => *emailp = ::std::ptr::null_mut(),
        Err(err) => {
            use crate::MoveIntoRaw;
            let status = crate::error::Status::from(&err);
            if let Some(errp) = errp {
